...
const AvailableTimingsSchema = new mongoose_1.Schema({
    dates: [String],
});
...
UserSchema.methods.generateToken = async function () {
    ...
    if (!token) {
        logger_1.default.error(`JwtError :: An error occurred while creating JwtToken`);
    ...
    if (!decodedData || typeof decodedData === "string") {
        logger_1.default.error(`JwtError :: An error occurred while decoding JwtToken`);
        throw new Error(strings_1.default.JWT_TOKEN_CREATE_ERROR);
    }
    ...
    if (!authToken) {
        logger_1.default.error(`AuthToken :: An error occurred while creating AuthToken`);
        throw new Error(strings_1.default.JWT_TOKEN_CREATE_ERROR);
    }
    logger_1.default.info(`AuthToken :: created`);
    return authToken;
};
...
UserSchema.methods.checkToken = async function (token) {
    const jwtSecret = LocalConfig_1.default.getConfig().JWT_SECRET;
    if (!jwtSecret)
        throw new Error(strings_1.default.JWT_SECRET_NOT_FOUND);

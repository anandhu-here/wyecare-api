...
class MailServiceHelper {
    static async sendEmail({ to, cc, bcc, subject, textContent, htmlContent, category, content, }) {
        ...
        if (!sendgridApiKey) {
            logger_1.default.error("Env :: SendGrid API key not found");
            throw new Error("SendGrid API key not found in ENV file");
        }
        if (!smtpFrom) {
            logger_1.default.error("Env :: SMTP From not found");
            throw new Error("SMTP From not found in ENV file");
        }
        mail_1.default.setApiKey(sendgridApiKey);
        if (!to) {
            logger_1.default.error("Mail reciever is required");
            throw new Error("Mail reciever is required");
        }
        if (!subject) {
            logger_1.default.error("Mail subject is required");
            throw new Error("Mail subject is required");
        }
        if (!htmlContent && !textContent && !content) {
            logger_1.default.error("Mail content is required");
            throw new Error("Mail content is required");
        }
        logger_1.default.info("SendGrid :: Sending email...");
        if (textContent) {
            return await mail_1.default
                ...
                logger_1.default.info(`${strings_1.default.EMAIL_SEND_SUCCESS} to ${to}`);
            }, (err) => {
